...
def trapezodial(x,y):
    ...
    for i in range(len(x)-1):
        ...
        areaq1.append(area)
        for i in areaq1:
            weights.append = ramp_mass - areaq1[i]
            fuel_mass = fuel_mass - area
    return area,areaq1,weights, Cmtotal, xcg

#weight = ramp_mass - trapezodial(#mf, #dt)
weight = ramp_mass - (fuel_mass[-1]-fuel_mass[-2])
cmtotal.append(Cmramp + Cms1 + Cms2 + Cms3 + Cms4 + Cms5 + Cms6 + Cms7 + Cms8 + Cms10 + Cmcab1 + Cmcab2 + Cmnose + Cmfuel[-1] + Cms9)
xcg.append(cmtotal[-1] / weight)
print(cmtotal[-1],xcg[-1])